...
func CriarUsuario(w http.ResponseWriter, r *http.Request) {
	...
	if erro != nil {
		w.Write([]byte("Erro ao criar o statement no banco de dados"))
		return
	}
	...
	if erro != nil {
		w.Write([]byte("Erro ao executar o statement no banco de dados"))
		return
	}
	...
	w.Write([]byte(fmt.Sprintf("usuario id: %d", idInserido)))
}
...
func BuscarUsuarios(w http.ResponseWriter, r *http.Request) {
	...
	for linhas.Next() {
		...
		if erro := linhas.Scan(&usuario.ID, &usuario.Nome, &usuario.Email); erro != nil {
			w.Write([]byte("Erro escanear o usuário"))
			return
		}
